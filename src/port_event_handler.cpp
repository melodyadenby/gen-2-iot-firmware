...
void PortEventHandler::handleVINMessage(const ParsedCANMessage &message) {
  ...
  // Check if VIN is complete (17 characters for full VIN)
  if (strlen(state->VIN) >= 17) {
    ...
    // Trigger cloud VIN validation for complete VIN
    handleVINValidation(port, state->VIN);
  } else {
...
void PortEventHandler::handleVINValidation(int port, const char *vin) {
  // Only validate complete VINs (17 characters)
  if (strlen(vin) < 17) {
    Serial.printlnf("VIN too short for validation: %s (%d chars)", vin,
                    strlen(vin));
    return;
  }

  // Prepare VIN for cloud validation
  char vinMessage[64];
  snprintf(vinMessage, sizeof(vinMessage), "VIN_VALIDATE,%d,%s", port, vin);

  publishStatusToCloud(port, vinMessage);

  Serial.printlnf("VIN validation request sent for port %d: %s", port, vin);
}
