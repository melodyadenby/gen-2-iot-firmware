...
void can_interrupt()
{
  ...
  if (readin == ERROR_OK)
  {
    // Quick corruption check in interrupt context
    if ((int32_t)recMsg.can_id < 0 || recMsg.can_id > MAX_PORTS ||
        recMsg.can_dlc > 8)
    {
      // Don't queue corrupted messages
      logCANError(0xF, "interrupt_corruption");
      return;
    }
