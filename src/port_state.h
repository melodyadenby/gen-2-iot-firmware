...
const unsigned long STUCK_VEHICLE_LIMIT =
    1500; // If we recieve a docking event STUCK_VEHICLE_LIMIT after state->
          // last_unlock_success then, this vehicle was not pulled out in time
const int STUCK_VEHICLE_RETRY_LIMIT =
    3; // Maximum number of retries for stuck vehicle
const unsigned long STUCK_VEHICLE_RETRY_DELAY =
    500; // 2.5 seconds delay between stuck vehicle unlock attempts
...
struct PortState {
  ...
  unsigned long last_unlock_success = 0; // Last successful unlock time
  int stuck_unlock_retry_count =
      0; // how many times we attempted to release stuck vehicle
  unsigned long stuck_vehicle_unlock_timer = 0; // Timer for next stuck vehicle unlock attempt
  bool stuck_vehicle_unlock_pending = false; // Flag indicating a stuck vehicle unlock is scheduled
